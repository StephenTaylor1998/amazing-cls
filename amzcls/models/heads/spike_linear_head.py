...
# @MODELS.register_module()
# class TALinearClsHead(LinearClsHead):
#
#     def __init__(self,
#                  num_classes: int,
#                  in_channels: int,
...
#             init_cfg = dict(type='Normal', layer='Linear', std=0.01)
#
#         super(TALinearClsHead, self).__init__(num_classes, in_channels, init_cfg, **kwargs)
#         self.enable_time_embed = enable_time_embed
#         if enable_time_embed:
#             # weight decay should be considered.
#             self.embed = nn.Parameter(torch.zeros(time_step))
...
class TALinearClsHead(LinearClsHead):
    ...
    def loss(self, feats: Tuple[torch.Tensor], data_samples: List[DataSample],
        ...
        for window_size, time_weight in zip(self.window_sizes, self.time_weights):
            ...
            time, batch, dim = cls_score.shape
            ta_loss = self._get_loss(cls_score.view(time * batch, dim), data_samples * time, **kwargs)
            # merge loss
            losses['loss'] = ta_loss['loss'] * time_weight

        # compute accuracy
        if self.cal_acc:
            losses.update({f'accuracy_top-{topk}': accuracy for topk, accuracy in zip(
                self.topk,
                compute_accuracy(cls_score, data_samples, self.topk)
            )})
        return losses
...
def compute_accuracy(cls_score, data_samples, topk: Union[int, Tuple[int]]):
    ...
    return Accuracy.calculate(cls_score, target, topk=topk)
